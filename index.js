...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN: ZAMZAM XD\n' // NAMA KONTAK OWNER
            + 'ORG:Creator ZAMZAM;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=6285668190592:+62 856-6819-0592\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '#'
blocked = ['6285668190592@s.whatsapp.net']
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `BYE ANAK PUNGUT😂 @${num.split('@')[0]}JANGAN BALIK LAGI KALO UDAH KELUAR AJG👋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285668190592@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6285668190592@s.whatsapp.net"]
			const adminbotnumber = ["6285668190592@s.whatsapp.net"]
			const frendsowner = ["6285668190592@s.whatsapp.net"]
			const premium = ["6285668190592@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMIN Z - BOT ✨* 」\n*│+wa.me/6281389368689*╰──────*「 *ZAMZAM* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  Z - BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281389368689*╰──────*「 * ZAMZAM* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  Z - BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
		...
		case 'iri':
            ...
            break
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					break
                case 'fakereplay':
                   client2.fakeReply("YAHAHA", "mending lari", "0823-877101916", MessageType.text)
                   break
				case 'infogc':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                ...
                if (!isPublic) return reply(mess.only.public)
					...
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar ZamZam|17`)
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                break
                case 'P':
               client.reply(from, `UTAMAKAN SALAM ${pushname}:)`)
                break
                case 'speed':
                case 'ping':
                if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6281389368689@s.whatsapp.net', options, text, {quoted: mek})
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
			...
			if (!isPublic) return reply(mess.only.public)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  Z - BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * ZAMZAM Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
